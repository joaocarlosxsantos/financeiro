...
interface Categoria { id: string; name: string; type: string; }
interface Carteira { id: string; name: string; }
...
export default function RendasUnificadas({ currentDate }: { currentDate: Date }) {
  ...
  const [form, setForm] = useState({ description: '', amount: '', date: '', categoryId: '', walletId: '', tags: [] as string[], isFixed: false });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      setShowForm(false);
      setEditingId(null);
      setForm({ description: '', amount: '', date: '', categoryId: '', walletId: '', tags: [], isFixed: false });
      setErrors({});
      // Recarrega rendas
      const year = currentDate.getFullYear();
      const month = currentDate.getMonth();
  ...
  return (
    <>
      <div className="space-y-4 px-2 sm:px-0">
        ...
        {showForm && (
          <Modal open={showForm} onClose={() => { setShowForm(false); setEditingId(null); }} title={editingId ? 'Editar Entrada' : 'Nova Entrada'}>
            <form className="space-y-3" onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 sm:gap-4">
                ...
                <div>
                  ...
                  <select id="category" className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={form.categoryId} onChange={e => {
                ...
                <div>
                  ...
                  <select id="wallet" className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={form.walletId} onChange={e => {
                ...
                <div>
                  ...
                  <select id="tag" className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={form.tags[0] || ''} onChange={e => {
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  />
                  <Label htmlFor="isFixed" className="ml-1 select-none cursor-pointer text-sm">Renda Fixa?</Label>
                </div>
              </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Entradas</h1>
            <p className="text-gray-600 dark:text-foreground">Gerencie todas as suas rendas (fixas e variáveis) do mês</p>
          </div>
          <Button onClick={() => { setShowForm(true); setEditingId(null); setForm({ description: '', amount: '', date: '', categoryId: '', walletId: '', tags: [], isFixed: false }); }}>
              ...
              <tbody>
                {filteredRendas.map((renda) => (
                  <tr key={renda.id} className="border-b hover:bg-accent transition-colors">
                    ...
                    <td className="px-3 py-2 text-center">
                      {renda.isFixed ? <span className="inline-block w-3 h-3 rounded-full bg-primary" title="Fixa" /> : null}
                    </td>
                    <td className="px-3 py-2 text-center">
