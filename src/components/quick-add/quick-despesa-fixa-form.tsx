...
export function QuickDespesaFixaForm({ onSuccess }: { onSuccess?: () => void }) {
  type FormState = { description: string; amount: string; dayOfMonth: string; categoryId: string; walletId: string; startDate: string; endDate: string; tags: string[] };
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <Input id="dayOfMonth" type="number" min="1" max="31" value={form.dayOfMonth} onChange={(e: ChangeEvent<HTMLInputElement>) => setForm((f: FormState) => ({ ...f, dayOfMonth: e.target.value }))} />
          ...
          <div>
            ...
            <Input id="startDate" type="date" value={form.startDate} onChange={(e: ChangeEvent<HTMLInputElement>) => setForm((f: FormState) => ({ ...f, startDate: e.target.value }))} />
          </div>
          <div>
            <Label htmlFor="endDate">Fim</Label>
            <Input id="endDate" type="date" value={form.endDate} onChange={(e: ChangeEvent<HTMLInputElement>) => setForm((f: FormState) => ({ ...f, endDate: e.target.value }))} />
          </div>
        </div>
