      ...
      <CardContent>
        <div className="space-y-6">
          ...
          <div className="relative bg-muted/30 rounded-lg p-4">
              ...
              {chartData.map((item, index) => (
                <div key={item.month} className="flex flex-col items-center flex-1 group h-full relative">
                  ...
                  >
                    ...
                    {item.score > 10 && (
                      <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-xs font-bold text-foreground/90">
                        {item.score}
                      </span>
                    )}
                  </div>
                  {/* Mês */}
                  <div className="absolute bottom-0 left-1/2 -translate-x-1/2 text-xs text-muted-foreground mt-3 text-center font-medium">
                    {item.displayMonth}
                  </div>
                </div>
              ))}
            ...
            <div className="absolute top-4 left-0 right-0 border-t border-dashed border-border opacity-50"></div>
            <div className="absolute top-2 right-2 text-xs text-muted-foreground">100</div>
            
            {/* Linha de referência para 50 pontos */}
            <div className="absolute top-1/2 left-0 right-0 border-t border-dashed border-border opacity-30 translate-y-2"></div>
            <div className="absolute top-1/2 right-2 text-xs text-muted-foreground translate-y-2">50</div>
            
            {/* Linha de referência para 0 pontos */}
            <div className="absolute bottom-4 left-0 right-0 border-t border-dashed border-border opacity-20"></div>
            <div className="absolute bottom-2 right-2 text-xs text-muted-foreground">0</div>
          </div>

          {/* Legenda melhorada */}
          <div className="flex flex-wrap justify-center gap-4 text-xs bg-muted/50 rounded-lg p-3">
