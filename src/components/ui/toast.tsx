...
export function Toast({ open, message, onClose, duration = 3000, inline = false }: ToastProps) {
  ...
  const baseClass = inline
    ? 'absolute top-4 right-4 z-50 bg-foreground text-background px-4 py-2 rounded shadow-lg animate-in fade-in'
    : 'fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-foreground text-background px-6 py-3 rounded shadow-lg animate-in fade-in';
