  ...
  return (
    <>
      <div className="space-y-4 px-2 sm:px-0">
        ...
        {showForm && (
          <Modal open={showForm} onClose={() => { setShowForm(false); setEditingId(null); }} title={editingId ? 'Editar Saída' : 'Nova Saída'}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 dark:text-foreground">Gerencie todas as suas despesas (fixas e variáveis) do mês</p>
          </div>
              ...
              <tbody>
                {filteredDespesas.map((despesa) => (
                  <tr key={despesa.id} className="border-b hover:bg-accent transition-colors">
                    ...
                    <td className="px-3 py-2 text-center">
                      {despesa.isFixed ? <span className="inline-block w-3 h-3 rounded-full bg-primary" title="Fixa" /> : null}
                    </td>
                    <td className="px-3 py-2 text-center">
