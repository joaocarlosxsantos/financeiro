...
export default function DespesasVariaveisTab({ currentDate }: any) {
  ...
  <div className="space-y-4 px-2 sm:px-0">
      ...
      {showForm && (
        <Modal open={showForm} onClose={() => { setShowForm(false); setEditingId(null); }} title={editingId ? 'Editar Despesa Variável' : 'Nova Despesa Variável'}>
          <form className="space-y-3" onSubmit={async (e) => {
            ...
            if (res.ok) {
              ...
              const start = new Date(year, month, 1).toISOString().slice(0, 10);
              const end = new Date(year, month + 1, 0).toISOString().slice(0, 10);
              const despesasRes = await fetch(`/api/expenses?type=VARIABLE&start=${start}&end=${end}`, { cache: 'no-store' });
              if (despesasRes.ok) {
                const data = await despesasRes.json();
                setDespesas(data.map((e: any) => ({
                  id: e.id,
