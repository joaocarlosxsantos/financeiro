...
export function SummaryRatioChart({ totalIncome, totalExpenses }: SummaryRatioChartProps) {
  ...
  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
        ...
        <div className="rounded-md border p-3 text-center">
          ...
          <p className="font-semibold text-red-600">{safeExpenses.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p>
        </div>
        <div className="rounded-md border p-3 text-center">
      ...
      {safeExpenses > safeIncome && (
        <p className="text-xs text-red-600">Atenção: despesas superiores à renda ({(safeExpenses / (safeIncome || 1) * 100).toFixed(1)}%).</p>
      )}
    </div>
  )
}
