...
export function DailyCategoryChart({ data, categoryColors, height = 320 }: DailyCategoryChartProps) {
  ...
  const DayBreakdownTooltip = ({ active, payload, label }: any) => {
    ...
    // Só adiciona se ainda não existir uma chave chamada 'Total' nos dados
    if (!items.find((it: any) => it.name === totalName)) {
      items.push({ name: totalName, value: total, color: categoryColors?.[totalName] || 'hsl(var(--muted-foreground))' });
    }
