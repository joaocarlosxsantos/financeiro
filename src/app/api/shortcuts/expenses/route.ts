...
export async function GET(req: NextRequest) {
  ...
  const user = await findUserFromSessionOrApiKey(req);
    // Return select data for the quick-add form: categories (BOTH|EXPENSE), wallets, tags
    const [categories, wallets, tags] = await Promise.all([
      prisma.category.findMany({ where: { userId: user.id, OR: [{ type: 'EXPENSE' }, { type: 'BOTH' }] }, orderBy: { name: 'asc' } }),
