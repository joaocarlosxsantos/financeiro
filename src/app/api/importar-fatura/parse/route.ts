...
import { identifyCreditCardCredit, identifyMerchant, suggestCategoryOnly } from '@/lib/merchant-dictionary';
...
async function suggestCategories(transactions: any[], existingCategories: any[], user: any, learningMap: any[], historyAvailable: boolean): Promise<any[]> {
  ...
  for (const t of transactions) {
    ...
    try {
      ...
      if (historyAvailable && learningMap.length > 0) {
        ...
        if (smartSuggestion.confidence >= 50) {
          ...
          descricaoMelhorada = altaConfiancaDescricao || t.descricao;
    ...
    // 3. FALLBACK: dicionário unificado de estabelecimentos/categorias (o mesmo
    // usado pelo extrato e por ai-categorization.ts) se smart e IA não
    // funcionaram OU se a IA retornou "Outros"
    if (suggestionSource === 'none') {
      ...
      if (isCredito) {
        ...
        const creditMatch = identifyCreditCardCredit(t.descricao);
        if (creditMatch) {
          categoriaSugerida = creditMatch.category;
        } else {
          const merchantMatch = identifyMerchant(t.descricao);
          if (merchantMatch) {
...
// Nota: o reconhecimento de estabelecimento/categoria por regras agora e
// 100% centralizado em '@/lib/merchant-dictionary' (identifyMerchant,
// identifyCreditCardCredit, suggestCategoryOnly) -- o mesmo modulo usado
// pelo parser de extrato e por ai-categorization.ts. As antigas funcoes
// locais sugerirCategoria/matchesCategoryKeywords/sugerirCategoriaPorRegrasFatura/
// sugerirCategoriaPorRegras (dicionarios duplicados e, no caso das duas
// primeiras, codigo morto sem nenhum call site) foram removidas nesta
// consolidacao.
